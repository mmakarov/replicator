...
- **GUI and CLI** ‚Ä a PySide6 desktop window for everyday use and a scriptable
  `replicator.py` command.
- **Automatic looping** ‚Ä the concatenated video repeats until it covers the
  full audio track, then the audio is muxed in.
- **Transparent overlay** ‚Ä any 1280x720 PNG overlay is composited over every
  frame.
- **Four text fields** ‚Ä heading, name, extra line, and date, rendered with
  bundled Noto Sans fonts (including Cyrillic).
- **Portable Windows build** ‚Ä a single `Replicator-Windows.zip` with embedded
  Python, Qt, ffmpeg, fonts, and sample media. No installation required.
- **Diagnostics** ‚Ä startup and render errors are written to `startup.log` and
...
4. Choose one or more videos, one audio file, and optionally a different PNG
   overlay.
5. Fill in the four text fields and click **ÇÄÇ**.
...
```text
source videos ‚Ä‚Ä‚ scale + crop to 1280x720 ‚Ä‚Ä‚ concat ‚Ä‚Ä‚
                                                        ‚‚Ä‚Ä‚ overlay + drawtext ‚Ä‚Ä‚ medium.mp4
overlay.png ‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚Ä‚
        ‚Ç
        ‚‚Ä‚Ä‚ loop to match audio length ‚Ä‚Ä‚ mux audio ‚Ä‚Ä‚ youtube_ready.mp4
```
...
1. Open **Actions** ‚Ü **Build Windows release**.
...
- **Nothing happens / app closes** ‚Ä check `startup.log` next to
  `run-windows.bat`.
- **Render fails** ‚Ä check `render.log`; it contains the full `ffmpeg` output.
- **`ffmpeg` not found** ‚Ä install ffmpeg and make sure it is on `PATH`, or
  place `ffmpeg`/`ffprobe` in a local `bin/` folder.
- **Not enough disk space** ‚Ä the app needs room for the intermediate video, the
  looped copy, and the final file; free several gigabytes before rendering.
- **Text is missing or garbled** ‚Ä keep the bundled `fonts/` folder next to the
